...
export class TestHelpers {
  ...
  static generateStudentData(overrides: Partial<any> = {}) {
    return {
      ...
      cpf: `${Math.floor(Math.random() * 100)}.${Math.floor(Math.random() * 1000).toString().padStart(3, '0')}.${Math.floor(Math.random() * 1000).toString().padStart(3, '0')}-${Math.floor(Math.random() * 100).toString().padStart(2, '0')}`,
      phone: "(11) 99999-9999",
